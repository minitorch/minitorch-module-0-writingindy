...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert minitorch.operators.sigmoid(a) <= 1.0 and minitorch.operators.sigmoid(a) >= 0.0
    assert_close(1 - minitorch.operators.sigmoid(a), minitorch.operators.sigmoid(-a))
    assert_close(minitorch.operators.sigmoid(0.0), 0.5)
    assert minitorch.operators.sigmoid(a + 0.1) >= minitorch.operators.sigmoid(a)
...
def test_sum(ls: List[float]) -> None:
    assert_close(minitorch.operators.sum(ls), sum(ls))
